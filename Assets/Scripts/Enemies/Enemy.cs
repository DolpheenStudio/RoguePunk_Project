...
public class Enemy : MonoBehaviour
{
    ...
    void Death()
    {
        for(int i=0; i<(int) (Random.Range(10,15) + Random.Range(10, 15) * (PlayerUpgrade.playerLevelIteration - 1) * 0.12f); i++)
        {
            if(Random.value > 0.66) Instantiate(scrapPrefab, new Vector3(transform.position.x + Random.Range(-1f, 1f), 0.12f, transform.position.z + Random.Range(-1f, 1f)), 
                                    Quaternion.Euler(-90f, transform.rotation.y, transform.rotation.z));
            else if(Random.value > 0.66) Instantiate(boltPrefab, new Vector3(transform.position.x + Random.Range(-1f, 1f), 0.12f, transform.position.z + Random.Range(-1f, 1f)), 
